...
int64 nChainStartTime = 1404652057;
...
const string strMessageMagic = "Plutonium Signed Message:\n";
...
int64 GetProofOfWorkReward(int nHeight, int64 nFees, uint256 prevHash)
{
    ...
    if (nHeight == 1)
        ...
        nSubsidy = 5 * COIN;
    else if ((nHeight > 100 && nHeight <= 500) || (nHeight > 8080 && nHeight <= 8580) || (nHeight > 1000 && nHeight <= 1600))
        nSubsidy = 12 * COIN;
    else if ((nHeight > 500 && nHeight <= 1000) || (nHeight > 6125 && nHeight <= 6225))
        nSubsidy = 15 * COIN;
    else if ((nHeight > 1600 && nHeight <= 2200))
        nSubsidy = 11 * COIN;
    else if ((nHeight > 2200 && nHeight <= 3200) || (nHeight > 5125 && nHeight <= 6125) || (nHeight > 6225 && nHeight <= 7025) || (nHeight > 7550 && nHeight <= 8050))
        nSubsidy = 10 * COIN;
    else if ((nHeight > 3200 && nHeight <= 3800) || (nHeight > 3850 && nHeight <= 4450) || (nHeight > 4475 && nHeight <= 5075) || (nHeight > 7025 && nHeight <= 7525)  || (nHeight > 8605 && nHeight <= 9105))
        nSubsidy = 5 * COIN;
    else if ((nHeight > 3800 && nHeight <= 3850) || (nHeight > 5075 && nHeight <= 5125))
        nSubsidy = 47 * COIN;
    else if ((nHeight > 8050 && nHeight <= 8080) || (nHeight > 7525 && nHeight <= 7550) || (nHeight > 4450 && nHeight <= 4475))
        nSubsidy = 94 * COIN;
    else if ((nHeight > 8580 && nHeight <= 8605) || (nHeight > 9105 && nHeight <= 9145))
        nSubsidy = 244 * COIN;
...
int64 GetProofOfStakeReward(int64 nCoinAge, unsigned int nBits, unsigned int nTime, int nHeight)
{
    ...
    nRewardCoinYear = MAX_RODS_PROOF_OF_STAKE;
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in Plutonium
    bool fStrictPayToScriptHash = true; // Always active in Plutonium
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Plutonium", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "6 July 2014";
        CTransaction txNew;
        ...
        assert(block.hashMerkleRoot == uint256("0xec99435ad49df4a4298b82b3227b868aa446158fb655f6b099bcdfaa915d195e"));
